...
struct pg_type : sqltype {
  ...
  pg_type(string name, OID oid, char typdelim, OID typrelid, OID typelem, OID typarray, char typtype)
      : sqltype(name), oid_(oid), typdelim_(typdelim), typrelid_(typrelid), typelem_(typelem), typarray_(typarray), typtype_(typtype) {}
...
struct schema_pqxx : public schema {
  ...
  virtual std::string quote_name(const std::string &id) { return c.quote_name(id); }
