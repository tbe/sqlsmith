...
struct truth_value : bool_expr {
  ...
  truth_value(prod *p) : bool_expr(p) { op = ((d6() < 4) ? scope->schema->true_literal : scope->schema->false_literal); }
};
...
struct null_predicate : bool_expr {
  ...
  virtual void out(std::ostream &out) { out << *expr << " is " << negate << "NULL"; }
  virtual void accept(prod_visitor *v) {
    v->visit(this);
    expr->accept(v);
  }
...
struct distinct_pred : bool_binop {
  ...
  virtual void out(std::ostream &o) { o << *lhs << " is distinct from " << *rhs; }
};
...
struct comparison_op : bool_binop {
  ...
  virtual void out(std::ostream &o) { o << *lhs << " " << oper->name << " " << *rhs; }
};
