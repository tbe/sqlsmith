...
shared_ptr<table_ref> table_ref::factory(prod *p)
{
  try {
    if (p->level < 3 + d6()) {
      if (d6() > 3 && p->level < d6())
        return make_alloc_shared<table_subquery>(p);
      if (d6() > 3)
        return make_alloc_shared<joined_table>(p);
    }
    if (d6() > 3)
      return make_alloc_shared<table_or_query_name>(p);
    else
      return make_alloc_shared<table_sample>(p);
  }
...
table_or_query_name::table_or_query_name(prod *p)
    : table_ref(p)
{
  t = random_pick(scope->tables);
  refs.push_back(make_alloc_shared<aliased_relation>(scope->stmt_uid("ref"), t));
}
...
target_table::target_table(prod *p, table *victim)
    : table_ref(p)
{
  while (!victim
  ...
  victim_ = victim;
  refs.push_back(make_alloc_shared<aliased_relation>(scope->stmt_uid("target"), victim));
}
...
table_sample::table_sample(prod *p)
    : table_ref(p)
{
  match();
  ...
  refs.push_back(make_alloc_shared<aliased_relation>(scope->stmt_uid("sample"), t));
  percent = 0.1 * d100();
...
{
  query = make_alloc_shared<query_spec>(this, scope, lateral);
  string alias = scope->stmt_uid("subq");
  relation *aliased_rel = &query->select_list->derived_table;
  refs.push_back(make_alloc_shared<aliased_relation>(alias, aliased_rel));
}
...
shared_ptr<join_cond> join_cond::factory(prod *p, table_ref &lhs, table_ref &rhs)
{
  try {
    if (d6() < 6)
      return make_alloc_shared<expr_join_cond>(p, lhs, rhs);
    else
      return make_alloc_shared<simple_join_cond>(p, lhs, rhs);
  }
  catch (runtime_error &e) {
    p->retry();
  }
...
joined_table::joined_table(prod *p)
    : table_ref(p)
{
...
from_clause::from_clause(prod *p)
    : prod(p)
{
  ...
  while (d6() > 5) {
    ...
    if (!impedance::matched(typeid(lateral_subquery)))
      break;
    reflist.push_back(make_alloc_shared<lateral_subquery>(this));
    for (const auto &r: reflist.back()->refs)
      scope->refs.push_back(&*r);
  }
}
    ...
    prod(p), myscope(s)
{
  ...
  from_clause = make_alloc_shared<struct from_clause>(this);
  select_list = make_alloc_shared<struct select_list>(this);
...
set_list::set_list(prod *p, table *target)
    : prod(p)
{
  do {
    ...
    : modifying_stmt(p, s, v)
{
  scope->refs.push_back(victim);
  search = bool_expr::factory(this);
  set_list = make_alloc_shared<struct set_list>(this, victim);
}
...
shared_ptr<prod> statement_factory(struct scope *s)
{
  try {
    s->new_stmt();
    if (d42() == 1)
      return make_alloc_shared<merge_stmt>((struct prod *) 0, s);
    if (d42() == 1)
      return make_alloc_shared<insert_stmt>((struct prod *) 0, s);
    else if (d42() == 1)
      return make_alloc_shared<delete_returning>((struct prod *) 0, s);
    else if (d42() == 1) {
      return make_alloc_shared<upsert_stmt>((struct prod *) 0, s);
    }
    else if (d42() == 1)
      return make_alloc_shared<update_returning>((struct prod *) 0, s);
    else if (d6() > 4)
      return make_alloc_shared<select_for_update>((struct prod *) 0, s);
    else if (d6() > 5)
      return make_alloc_shared<common_table_expression>((struct prod *) 0, s);
    return make_alloc_shared<query_spec>((struct prod *) 0, s);
  }
  catch (runtime_error &e) {
    ...
    : prod(parent), myscope(s)
{
  ...
  while (d6() > 3);
  try {
    query = make_alloc_shared<query_spec>(this, scope);
  }
  catch (runtime_error &e) {
    retry();
    goto retry;
...
{
  match();
  target_table_ = make_alloc_shared<target_table>(this, victim);
  data_source = table_ref::factory(this);
//   join_condition = join_cond::factory(this, *target_table_, *data_source);
  join_condition = make_alloc_shared<simple_join_cond>(this, *target_table_, *data_source);
    ...
    : when_clause(p), myscope(p->scope)
{
  ...
  set_list = make_alloc_shared<struct set_list>(this, p->victim);
}
...
shared_ptr<when_clause> when_clause::factory(struct merge_stmt *p)
{
  try {
    switch (d6()) {
      case 1:
      case 2:
        return make_alloc_shared<when_clause_insert>(p);
      case 3:
      case 4:
        return make_alloc_shared<when_clause_update>(p);
      default:
        return make_alloc_shared<when_clause>(p);
    }
  }
  catch (runtime_error &e) {
    p->retry();
