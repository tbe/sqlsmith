...
struct scope {
  ...
  void new_stmt() { stmt_seq = std::make_shared<map<string, unsigned int>>(); }
};
...
struct routine {
  ...
  sqltype          *restype;
  string            name;
  routine(string schema, string specific_name, sqltype *data_type, string name) : specific_name(specific_name), schema(schema), restype(data_type), name(name) {
    assert(data_type);
  }
