...
struct aliased_relation: named_relation
{
    ...
    {}
    aliased_relation(string n, relation *r)
        : named_relation(std::move(n)), rel(r)
    {}
    vector<column> &columns() override
    { return rel->columns(); }
};
...
struct scope
{
    ...
    std::pmr::vector<named_relation *> tables;
    /// available to column_ref productions
    std::pmr::vector<named_relation *> refs;
    struct schema *schema;
    /// Counters for prefixed stmt-unique identifiers
    shared_ptr<std::pmr::map<string, unsigned int> > stmt_seq;
    scope(struct scope *parent = nullptr)
        : parent(parent)
    {
      if (parent) {
    ...
    void new_stmt()
    {
      stmt_seq = std::make_shared<std::pmr::map<string, unsigned int> >(getAllocator<std::pair<const string,unsigned int> >());
    }
};
