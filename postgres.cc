...
schema_pqxx::schema_pqxx(std::string &conninfo, bool no_catalog) : c(conninfo) {
  ...
  for (auto row = r.begin(); row != r.end(); ++row) {
    ...
    tables.push_back(table(row[0].as<string>(), schema, ((insertable == "YES") ? true : false), ((table_type == "BASE TABLE") ? true : false)));
  }
  ...
  for (auto row : r) {
    op o(row[0].as<string>(), oid2type[row[1].as<OID>()], oid2type[row[2].as<OID>()], oid2type[row[3].as<OID>()]);
    register_operator(o);
  }
  ...
  for (auto row : r) {
    routine proc(row[0].as<string>(), row[1].as<string>(), oid2type[row[2].as<long>()], row[3].as<string>());
  ...
  for (auto row : r) {
    routine proc(row[0].as<string>(), row[1].as<string>(), oid2type[row[2].as<OID>()], row[3].as<string>());
...
dut_libpq::dut_libpq(std::string conninfo) : conninfo_(conninfo) { connect(conninfo); }
