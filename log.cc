...
struct stats_visitor : prod_visitor {
  ...
  void report() {
    ...
    for (auto p : production_stats)
      report.push_back(p);
    stable_sort(report.begin(), report.end(), [](const pair<std::string, long> &a, const pair<std::string, long> &b) { return a.second > b.second; });
    for (auto p : report) {
...
pqxx_logger::pqxx_logger(std::string target, std::string conninfo, struct schema &s) {
  ...
  c->prepare("instance", "insert into instance (rev, target, hostname, version, seed) "
  ...
  c->prepare("error", "insert into error (id, msg, query, sqlstate) "
                      "values (" +
                          to_string(id) + ", $1, $2, $3)");

  w.exec("insert into stat (id) values (" + to_string(id) + ")");
  c->prepare("stat", "update stat set generated=$1, level=$2, nodes=$3, updated=now() "
