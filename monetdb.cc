...
schema_monetdb::schema_monetdb(std::string &conninfo) : monetdb_connection(conninfo) {
  ...
  while (mapi_fetch_row(hdl)) {
    tables.push_back(table(mapi_fetch_field(hdl, 0), mapi_fetch_field(hdl, 1), strcmp(mapi_fetch_field(hdl, 2), "false") == 0 ? true : false,
  ...
  string routq("select s.name, f.id, a.type, f.name from sys.schemas s, sys.args a, sys.types t, sys.functions f where f.schema_id = s.id and f.id=a.func_id "
               "and a.number=0 and a.type = t.sqlname and f.mod<>'aggr'");
  ...
  string aggq("select s.name, f.id, a.type, f.name from sys.schemas s, sys.args a, sys.types t, sys.functions f where f.schema_id = s.id and f.id=a.func_id "
              "and a.number=0 and a.type = t.sqlname and f.mod='aggr'");
